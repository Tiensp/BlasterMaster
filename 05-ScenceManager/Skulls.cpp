...
void CSkull::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
	...
	else //có va chạm
	{
		...
		/*if (abs(this->y - target->y) <= 100)
		{
			if (abs(this->y - target->y) <= 50)
			{
					...
					if (e->nx != 0)
					{
						if (e->nx > 0)
						{
							if (this->nx < 0)
							{
								if (this->GetState() == SKULL_ANI_WALKING_LEFT)
								{
									this->SetState(SKULL_ANI_COLLISION_LEFT_UP);
								}
							}
						}
						else if(e->nx < 0)
						{
							if (this->nx > 0)
							{
								if (this->GetState() == SKULL_ANI_COLLISION_RIGHT)
								{
									this->SetState(SKULL_ANI_);
								}
							}
							else if (this->nx < 0)
							{
...
void CSkull::SetState(int state)
{
	...
	case SKULL_ANI_ATTACKING_RIGHT:
		...
		ny = 0;
		break;
	case SKULL_ANI_COLLISION_LEFT_UP:
		vx = 0;
		nx = -1;
		vy = -SKULL_JUMPING_SPEED;
		ny = -1;
		break;
	case SKULL_ANI_COLLISION_RIGHT_UP:
		vx = 0;
		nx = 1;
		vy = -SKULL_JUMPING_SPEED;
		ny = -1;
